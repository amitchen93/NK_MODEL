...
def test2():
    ...
    k = 4
    local_fitness_values = local_fitness_values_dict(N, k)
    autocorr_res = autocorr(get_random_genotype_fitness_over_trajectory(trajectory_length=2 ** 5, N=N, k=k,
...
def test_i():
    ...
    for k in Ks:
        ...
        plt.plot(auto_corr_cum[:32] / num_repeats, label=f'{k}')
    plt.legend()
